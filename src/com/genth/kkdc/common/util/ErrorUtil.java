...
import com.genth.kkdc.KKDCConstant;
import com.genth.kkdc.exception.*;
...
public class ErrorUtil {
	...
	public static void handleCommonException(SQLException e) throws CommonException {
		...
		if (e.getErrorCode() == 54
		           || resolver.getProperty(KKDCConstant.ERR_CONCURRENT_LOCK).equals(e.getMessage())) {
			throw ErrorUtil.generateError(resolver.getProperty(KKDCConstant.ERR_CONCURRENT_LOCK),
			                              null);
		} else if (e.getMessage().toLowerCase().indexOf("io exception") > -1) {
			throw generateError(resolver.getProperty(KKDCConstant.CON_FAILED), null);
		} else {
			throw ErrorUtil.generateError(resolver.getProperty(KKDCConstant.UNDEFINED_ERR),
			                              e.getMessage());
		}
	}
